...
func (p *round3Handler) HandleMessage(logger log.Logger, message types.Message) error {
	...
	err = round3.Psidoublepai.Verify(parameter, peer.ssidWithBk, peer.round1Data.kOtherCiphertext, n, ownPed.Getn(), ownPed.Gets(), ownPed.Gett(), Delta, p.sumGamma)
	if err != nil {
		return err
	}
...
func (p *round3Handler) Finalize(logger log.Logger) (types.Handler, error) {
	...
	for _, peer := range p.peers {
		...
		delta.Add(delta, peer.round3Data.otherDelta)
