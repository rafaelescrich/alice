...
func (p *round2Handler) HandleMessage(logger log.Logger, message types.Message) error {
	...
	peer.round2Data = &round2Data{
		psiProof:      round2.Psi,
		otherD:        new(big.Int).SetBytes(round2.D),
		otherF:        new(big.Int).SetBytes(round2.F),
		psihatProoof:  round2.Psihat,
		otherDhat:     new(big.Int).SetBytes(round2.Dhat),
		otherFhat:     new(big.Int).SetBytes(round2.Fhat),
		otherAlpha:    new(big.Int).SetBytes(alpha),
		otherAlphahat: new(big.Int).SetBytes(alphahat),
		allGammaPoint: Gamma,
	}
...
func (p *round2Handler) Finalize(logger log.Logger) (types.Handler, error) {
	...
	for id, peer := range p.peers {
		...
		delta.Add(delta, peer.round2Data.otherAlpha)
		delta.Add(delta, peer.round1Data.beta)
		delta.Mod(delta, curveN)
		chi.Add(chi, peer.round2Data.otherAlphahat)
